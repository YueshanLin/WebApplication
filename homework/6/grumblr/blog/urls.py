...
urlpatterns = [
    ...
    path('reset', blog.views.reset, name='reset'),
    re_path('^password_reset_confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$', blog.views.password_reset_confirm, name='password_reset_confirm'),
    path('password_reset/<int:user_id>', blog.views.password_reset, name='password_reset'),
    ...
    re_path('^confirmation/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$', blog.views.verification, name='verification')
]
